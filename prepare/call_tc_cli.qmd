# Call tax-calculator command-line interface (tc-cli)
...
write_csv(df, fs::path(DDATA, "tc_data/hholds.csv"))
...
hhbase <- read_csv(fs::path(DDATA, "tc_data/hholds.csv"))
hhbase
hhbase |> filter(RECID == 12)
# e19800 is itemizable charitable giving

recn <- 12 # 74 is max agi; 28 is max iitax increase; 12 is max ddecrease
hhdf1 |>
  filter(RECID == recn) |>
  # calc saltsum because it is not shown if the person takes standard
  mutate(saltsum = e18400 + e18500) |> #  + e18800???
  mutate(dedused1 = pmax(standard, c04470), dedused2 = c00100 - c04800) |>
  select(
    RECID,
    year,
    FLPDYR,
